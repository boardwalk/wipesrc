...
void DrawTargetIcon(ShipData* playerShip, ShipData* shipIndex, Object** shipShapes, POLY_FT4* target) {
  ...
  if (!(flag & (CrtClipped | ZNegative | ZLarge | ZSmall))) {
    ...
    target->pad6 = 0;
    target->pad7 = 0;
    AddPrim(OT[CurrentScreen], 0, (P_TAG*)&target[CurrentScreen]);
  }
}
  ...
  if ((shipIndex[weapon->fireShip].attr & VIEW_IN) == 0) {
    ...
    for (k = 0; k < primitiveCount; k++) {
      ...
      case TypeG4:
        ...
        //   				AddPrim(OT[CurrentScreen], 0, prims.ptr );
        prims.g4 += 1;

        shieldID = weapon->currentWeapon - SHIELD_OFFSET;
        //      			AddPrim(OT[CurrentScreen], 0, ( P_TAG* ) &shieldParams[(64 * shieldID)][CurrentScreen]);
        prims.drmode += 1;
...
void DrawInternalShield() {
  if (!DrawnShieldBefore) {
    ...
    AddPrim(OT[CurrentScreen], 1, (P_TAG*)&shieldPoly); //Depth 1 to prevent hud from being affected
  }
  DrawnShieldBefore = 1;
}
...
void CycleWeaponGrids(ShipData* shipIndex, WeaponGrid* weaponGrid, int32_t noWeaponGrids) {
  ...
  colourCount++;
  if (colourCount == CYCLE_END)
    colourCount = CYCLE_START;
  for (i = 0; i < noWeaponGrids; i++) {
    ...
    for (j = 0; j < NO_OF_SHIPS; j++) {
      if ((shipIndex[j].combatAttr & SPECIALED) == 0) {
        if (((facePtr->flags & WEAPON_GRID_LEFT) && (shipIndex[j].attr & LSIDE))) {
          if ((weaponGrid[i].section == shipIndex[j].nearTrkSect) && (shipIndex[j].weaponType == NO_WEAPON) &&
    ...
    if ((weaponGrid[i].count >= 0) && ((shipIndex[ownShip].combatAttr & SPECIALED) == 0)) {
      ...
      if (tempColour.vx > 255)
        tempColour.vx = 255;
      if (tempColour.vy > 255)
        tempColour.vy = 255;
      if (tempColour.vz > 255)
        tempColour.vz = 255;
...
void SetTargetShip(ShipData* shipIndex, int32_t shipNo) {
  ...
  if (shortestDistance < 10) {
    ...
    shipIndex[shipNo].targetShip = 0xFF;
}
