...
void Ship2ShipCollision(ShipData* shipIndex, Object** shipShapes, int32_t i, int32_t j) {
  int32_t mag;
  VECTOR distance;
  int32_t collide;
  VECTOR vc;
  int32_t ownMass, remMass;
  VECTOR ship2ship;
...
int32_t collideLineToPlane(int32_t shipNo1, int32_t shipNo2, Object** shipShapes, ShipData* shipIndex) {
  int32_t i; /* index count (planes) */
  int32_t count; /* index count (lines) */
  Poly poly;
  int16_t* coords;
  ...
  int32_t factor2; /* intermediate calculation terms */
  VECTOR factor1;
  VECTOR term1;

  VECTOR res_vec; /* resultant intersection vector */
  int32_t alpha; /* the above alpha value */
...
int32_t PlaneLineCollision(VECTOR plane_point, VECTOR ship_pos, SVector normal) {
  int32_t dot_product, norm_dot_product;
  int32_t coll_data;
...
void Ship2Track(ShipData* playerShip, Face* facePtr, Object** shipShapes) {
  ...
  if (toFace > 0) {
    ...
    if ((facePtr == (track->faces + playerShip->nearTrkSect->faceStart)) || (playerShip->nearTrkSect->junction.index != 0)) {
      ...
      if (alpha <= 0) {
        if ((playerShip->nearTrkSect->flags & JunctionStart) || (playerShip->nearTrkSect->flags & JunctionEnd)) {
...
void Reflection(VECTOR* incidence, SVector normal) {
  ...
  temp = ((((int32_t)normal.vx * -incidence->vx) >> 12) +
          ((int32_t)(normal.vy * -incidence->vy) >> 12) +
          ((int32_t)(normal.vz * -incidence->vz) >> 12));

  incidence->vx = (((2 * (int32_t)normal.vx) * temp) >> 12) + incidence->vx;
  incidence->vy = (((2 * (int32_t)normal.vy) * temp) >> 12) + incidence->vy;
  incidence->vz = (((2 * (int32_t)normal.vz) * temp) >> 12) + incidence->vz;
}

int32_t AngleOfCollision(SVECTOR normal, VECTOR ship_point) {
  int32_t dot_product;
  int32_t norm_dist, shpoint_dist;
  int32_t dist_product;
