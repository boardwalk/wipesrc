...
short BlockError( Block* block )
{
/*
   if ( ( block < ( Block* )0x80000000 ) || ( block > ( Block* )0x80800000 ) )
   {
      return( 1 );
   }
...
)
{
   ...
   sh = ( StaticHeap* )DAlloc( dh, sizeof( StaticHeap ) );
...
/* Check if we need to allocate an empty block */

   if ( sh->free < ( size + sizeof( EmptyBlock ) ) )
   {
   /* Put this block on the empty list and allocate a new one */
      
      empty = ( EmptyBlock* ) sh->block;
      empty->next = sh->empty;
      sh->empty = empty;

      sh->block = DAlloc( sh->heap, sh->size );
      sh->mem = sh->block;
...
/* Allocate size bytes from this block */
...
/* Free up the empty blocks */
   ...
   while( block )
   {
      ...
      DFree( sh->heap, ( char* ) empty );
   }

/* Free the current block */

   DFree( sh->heap, ( char * )sh->block );

/* Free header block */

   DFree( sh->heap, ( char* )sh );
}
