...
//          4. C3D_ERS_SURF_DRAW_PITCH  | rendering states have not been tested 
//             C3D_ERS_SURF_DRAW_PF     |  
//          5. the view port(C3D_ERS_SURF_VPORT) starting address must be 8 byte  
//              alligned in the framebuffer.  It is therefore recommended that 
//              this feature not be used until this restriction is lifted 
//              in the Beta III release on the 20 Feburary 1996.

// NB: stuctures must be packed on 8 byte boundries.
//
...
typedef struct {                        
    C3D_UINT32 x, y, z;                 // FIXED point typed  
} C3D_V, * C3D_PV;                      // identified by C3D_EV_V

typedef struct { 
    C3D_UINT32 x, y, z;                 // FIXED point typed  
    C3D_UINT16 r, g, b, a;              // identified by C3D_EV_VC
} C3D_VC, * C3D_PVC;

typedef struct { 
    C3D_UINT32 x, y, z;                 // FIXED point typed  
    C3D_UINT32 s, t, w;                 // identified by C3D_EV_VT
} C3D_VT, * C3D_PVT;

typedef struct {                        
    C3D_UINT32 x, y, z;                 // FIXED point typed  
    C3D_UINT32 s, t, w;                 // identified by C3D_EV_VTC
    C3D_UINT16 r, g, b, a;              
} C3D_VTC, * C3D_PVTC;                  
...
// texel filtering
typedef enum {
    C3D_ETFILT_PICK_NEAREST     = 0,// selects pick nearest filtering
    C3D_ETFILT_BI_LINEAR        = 1,// selects bi-linear filtering
    C3D_ETFILT_NUM              = 2
...
typedef struct {
    ...
    C3D_BOOL        bMipMap;                    // is this a mip map ?
    C3D_PVOID       apvLevels[cu32MAX_TMAP_LEV];// array of pointer to map(s)
    ...
    C3D_BOOL        bTexTranspEnable;           // use texel transparency
    C3D_COLOR       clrTexTranspColor;          // specify texel transp. clr
} C3D_TMAP, * C3D_PTMAP;
...
typedef enum {                  
    ...
    C3D_ERS_TMAP_FILTER     = 9,    // C3D_ETEXFILTER   C3D_ETFILT_PICK_NEAREST
    C3D_ERS_ALPHA_SRC       = 10,   // C3D_EASRC        C3D_EASRC_ONE
    C3D_ERS_ALPHA_DST       = 11,   // C3D_EADST        C3D_EADST_ZERO
    C3D_ERS_SURF_DRAW_PTR   = 12,   // C3D_PVOID        Desktop Surface
    C3D_ERS_SURF_DRAW_PITCH = 13,   // C3D_UINT32       Desktop Surface
    C3D_ERS_SURF_DRAW_PF    = 14,   // C3D_EPIXFMT      Desktop Surface
    C3D_ERS_SURF_VPORT      = 15,   // C3D_RECT         Desktop Surface
    C3D_ERS_NUM             = 16    // invalid enumeration
} C3D_ERSID, * C3D_PERSID;

// 
// Inlines
//
#define SET_CIF_COLOR(exp, red, grn, blu, alf, fog)      \
    (exp).r             = (C3D_UINT8)(red);              \
    (exp).g             = (C3D_UINT8)(grn);              \
    (exp).b             = (C3D_UINT8)(blu);              \
    (exp).a             = (C3D_UINT8)(alf);              \
...
#ifdef RCS_H_KEYS
    static char *rcsid_h_ati3dcif_ati = "$Revision:   2.6  $" __FILE__;
#endif
...
// $Author:   tomp  $
// $Revision:   2.6  $
// $Date:   Feb 15 1996 00:23:22  $
// $Modtime:   Feb 15 1996 00:09:24  $
// $Log:   O:/common/_vcs/ati3dcif.h_v  $
// 
//    Rev 2.6   Feb 15 1996 00:23:22   tomp
// d6 release
// 
// optimized
//     Alpha Blender is turned off for the nop comibnation of SRC and DST functi
//     Alpha interpolators are not setup when neither the SRC nor the DST funcit
//     depend on source alpha
// 
// fixed
//     Texture Chorma Keying
//     
...
// Draw.  Replaced implict Lock/explicit Unlock with Begin/End Render calls
// Add support to select filtering mode.
// 
