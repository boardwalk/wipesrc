...
/* Calculate non-meshed track sections */
   ...
   for ( i=0; i < loCount; i++ )
   {
	  ...
	  for ( j=0; j < faceCount; j++ )
      {
		...
		RotTransPersN(svector, screen, depth2, flag2, 4, &zbuf[0]);
...
//            AddPrim( OT[ CurrentScreen ] + ( *depth ), ( ulong* ) ft4 );
            ...
            ClipPolyFT4( OT[ CurrentScreen ] + ( (depth2[0] >> DepthShift) - DepthMod), ( ulong* ) ft4 );
         }
...
TrackSection *AsmTransformTrackLoNoTex(TrackSection* firstSection,
					Skeleton* camera, short loCount)
{
/* 
   Variables with greatest need for optimisation 
   Listed in decreasing order of priority 
...
//		if ( !( *flag & ClipFlags ) )
         if ( !( ( flag2[ 0 ] | flag2[ 1 ] | flag2[ 2 ] | flag2[ 3 ] ) & ShortClipFlags ) )
         {
            f4 = (PolyF4 *)&( TrackLo[ CurrentScreen ][ TrackLoCount++ ] );
			SetPolyF4((POLY_F4 *)f4);
         ...
         /* Just use one corner for the depth info */
			...
			if (section->secNo != startLinePos[trackNo])
			          f4->colour0.r = face->red;
			else
			          f4->colour0.r = WhitePal;

			if(!(face->flags & TRACK_BASE) && Shielded)
				f4->colour0.r = (&DepthFadeTble)[(256*(WHITE6_TRANSL-1)) + f4->colour0.r];

            f4->colour0.g = 0;
            f4->colour0.b = 0;


            //AddPrim( OT[ CurrentScreen ] + ( (depth2[0] >> DepthShift) - DepthMod), ( ulong* ) f4 );
            ClipPolyF4( OT[ CurrentScreen ] + ( (depth2[0] >> DepthShift) - DepthMod), ( ulong* ) f4 );
         }
...
/* Calculate Med res meshed track sections */
   ...
   for ( i=0; i < medCount; i++ )
   {
      ...
      /* make SVECTORs */
         ...
         RotTransPersN( svector, screen, depth, flag, 9 , zbuf);
      ...
      /* Top left polygon */
         ...
         if ( !( ( flag[ 0 ] | flag[ 1 ] | flag[ 3 ] | flag[ 4 ] ) & ShortClipFlags ) )
         {
            ...
            ClipPolyFT4( OT[ CurrentScreen ] + ((depth[ 0 ] >> DepthShift) - DepthMod ), ( ulong* ) ft4 );
	   }
      ...
      /* Top right polygon */
         ...
         if ( !( ( flag[ 1 ] | flag[ 2 ] | flag[ 4 ] | flag[ 5 ] ) & ShortClipFlags ) )
         {
    	    ...
    	    //  AddPrim( OT[ CurrentScreen ] + (( depth[ 1 ] >> DepthShift) - DepthMod ), ( ulong* ) ft4 );
    	      ClipPolyFT4( OT[ CurrentScreen ] + (( depth[ 1 ] >> DepthShift) - DepthMod ), ( ulong* ) ft4 );
         }
      ...
      /* Bottom left polygon */
         ...
         if ( !( ( flag[ 3 ] | flag[ 4 ] | flag[ 6 ] | flag[ 7 ] ) & ShortClipFlags ) )
         {
            ...
            ClipPolyFT4( OT[ CurrentScreen ] + (( depth[ 3 ] >> DepthShift) - DepthMod ), ( ulong* ) ft4 );
         }
      ...
      /* Bottom right polygon */
         ...
         if ( !( ( flag[ 4 ] | flag[ 5 ] | flag[ 7 ] | flag[ 8 ] ) & ShortClipFlags ) )
         {
            ...
            ClipPolyFT4( OT[ CurrentScreen ] + (( depth[ 4 ] >> DepthShift) - DepthMod ), ( ulong* ) ft4 );
...
TrackSection *AsmTransformTrackMedNoTex
( 
   TrackSection*     firstSection,
   Skeleton*         camera,
   short             medCount,
...
/* Calculate Med res meshed track sections */
   ...
   for ( i=0; i < medCount; i++ )
   {
      ...
      /* get original 32 bit faces coords */
         ...
         vector[ 0 ] = *( vertices + face->vertex[ 1 ] );
         vector[ 1 ] = *( vertices + face->vertex[ 0 ] );
         vector[ 2 ] = *( vertices + face->vertex[ 2 ] );
         vector[ 3 ] = *( vertices + face->vertex[ 3 ] );
      ...
      /* make SVECTORs */
         ...
         RotTransPersN( svector, screen, depth, flag, 4 );


      /* Draw all as 1 polygon */

         if ( !( ( flag[ 0 ] | flag[ 1 ] | flag[ 2 ] | flag[ 3 ] ) & ShortClipFlags ) )
         {
            f4 = (PolyF4 *)&( TrackMed[ CurrentScreen ][ TrackMedCount++ ] );
            ...
            //AddPrim( OT[ CurrentScreen ] + ((depth[ 0 ] >> DepthShift) - DepthMod ), ( ulong* ) f4 );
            ClipPolyF4( OT[ CurrentScreen ] + ((depth[ 0 ] >> DepthShift) - DepthMod ), ( ulong* ) f4 );
	   }
       face++;
      }
...
/* Calculate Hi res meshed track sections */
   ...
   for ( i=0; i < highCount; i++ )
   {
      ...
      /* make SVECTORs */
         ...
         RotTransPersN( svector, screen, depth, flag, 25 , zbuf);
      ...
      /* polygon 2 */
         ...
         if (!( ( flag[ 2 ] | flag[ 3 ] | flag[ 7 ] | flag[ 8 ] ) & ShortClipFlags ) )
         {
            ...
            ClipPolyFT4( OT[ CurrentScreen ] + (( depth[ 2 ] >> DepthShift) - DepthMod  ), ( ulong* ) ft4 );
         }
      ...
      /* polygon 4 */
         ...
         if (!( ( flag[ 5 ] | flag[ 6 ] | flag[ 10 ] | flag[ 11 ] ) & ShortClipFlags ) )
         {
            ...
            ClipPolyFT4( OT[ CurrentScreen ] + (( depth[ 5 ] >> DepthShift) - DepthMod  ), ( ulong* ) ft4 );
	  }
      ...
      /* polygon 5 */
         ...
         if (!( ( flag[ 6 ] | flag[ 7 ] | flag[ 11 ] | flag[ 12 ] ) & ShortClipFlags ) )
         {
            ...
            ClipPolyFT4( OT[ CurrentScreen ] + (( depth[ 6 ] >> DepthShift) - DepthMod  ), ( ulong* ) ft4 );
         }
      ...
      /* polygon 6 */
         ...
         if (!( ( flag[ 7 ] | flag[ 8 ] | flag[ 12 ] | flag[ 13 ] ) & ShortClipFlags ) )
         {
            ...
            ClipPolyFT4( OT[ CurrentScreen ] + (( depth[ 7 ] >> DepthShift) - DepthMod  ), ( ulong* ) ft4 );
         }
      ...
      /* polygon 7 */
         ...
         if (!( ( flag[ 8 ] | flag[ 9 ] | flag[ 13 ] | flag[ 14 ] ) & ShortClipFlags ) )
         {
            ...
            ClipPolyFT4( OT[ CurrentScreen ] + (( depth[ 8 ] >> DepthShift) - DepthMod  ), ( ulong* ) ft4 );
	  }
      ...
      /* polygon 8 */
         ...
         if (!( ( flag[ 10 ] | flag[ 11 ] | flag[ 15 ] | flag[ 16 ] ) & ShortClipFlags ) )
         {
            ...
            ClipPolyFT4( OT[ CurrentScreen ] + (( depth[ 10 ] >> DepthShift) - DepthMod  ), ( ulong* ) ft4 );
         }
      ...
      /* polygon 9 */
         ...
         if (!( ( flag[ 11 ] | flag[ 12 ] | flag[ 16 ] | flag[ 17 ] ) & ShortClipFlags ) )
         {
            ...
            ClipPolyFT4( OT[ CurrentScreen ] + (( depth[ 11 ] >> DepthShift) - DepthMod  ), ( ulong* ) ft4 );
         }
      ...
      /* polygon 10 */
         ...
         if (!( ( flag[ 12 ] | flag[ 13 ] | flag[ 17 ] | flag[ 18 ] ) & ShortClipFlags ) )
         {
            ...
            ClipPolyFT4( OT[ CurrentScreen ] + (( depth[ 12 ] >> DepthShift) - DepthMod  ), ( ulong* ) ft4 );
	   }
      ...
      /* polygon 11 */
         ...
         if (!( ( flag[ 13 ] | flag[ 14 ] | flag[ 18 ] | flag[ 19 ] ) & ShortClipFlags ) )
         {
            ...
            ClipPolyFT4( OT[ CurrentScreen ] + (( depth[ 13 ] >> DepthShift) - DepthMod  ), ( ulong* ) ft4 );
	  }
      ...
      /* polygon 12 */
         ...
         if (!( ( flag[ 15 ] | flag[ 16 ] | flag[ 20 ] | flag[ 21 ] ) & ShortClipFlags ) )
         {
            ...
            ClipPolyFT4( OT[ CurrentScreen ] + (( depth[ 15 ] >> DepthShift) - DepthMod  ), ( ulong* ) ft4 );
	  }
      ...
      /* polygon 13 */
         ...
         if (!( ( flag[ 16 ] | flag[ 17 ] | flag[ 21 ] | flag[ 22 ] ) & ShortClipFlags ) )
         {
            ...
            ClipPolyFT4( OT[ CurrentScreen ] + (( depth[ 16 ] >> DepthShift) - DepthMod  ), ( ulong* ) ft4 );
         }
      ...
      /* polygon 14 */
         ...
         if (!( ( flag[ 17 ] | flag[ 18 ] | flag[ 22 ] | flag[ 23 ] ) & ShortClipFlags ) )
         {
            ...
            ClipPolyFT4( OT[ CurrentScreen ] + (( depth[ 17 ] >> DepthShift) - DepthMod  ), ( ulong* ) ft4 );
         }
      ...
      /* polygon 15 */
         ...
         if (!( ( flag[ 18 ] | flag[ 19 ] | flag[ 23 ] | flag[ 24 ] ) & ShortClipFlags ) )
         {
            ...
            ClipPolyFT4( OT[ CurrentScreen ] + (( depth[ 18 ] >> DepthShift) - DepthMod  ), ( ulong* ) ft4 );
         }
...
TrackSection *AsmTransformTrackHiNoTex1
( 
   TrackSection*     firstSection,
   Skeleton*         camera,
   AutoMeshData*     autoMesh,
...
/* Calculate Hi res meshed track sections */
   ...
   for ( i=0; i < highCount; i++ )
   {
      ...
      /* get original 32 bit faces coords */
		...
		vector[ 0 ] = *( vertices + face->vertex[ 1 ] );
		Test16bitX = vector[0].vx + campos->vx;
	    Test16bitY = vector[0].vy + campos->vy;
	    Test16bitZ = vector[0].vz + campos->vz;
		Test16Bit(&Test16bitX, &Test16bitY, &Test16bitZ);
		...
		vector[ 1 ] = *( vertices + face->vertex[ 0 ] );
		Test16bitX = vector[1].vx + campos->vx;
	    Test16bitY = vector[1].vy + campos->vy;
	    Test16bitZ = vector[1].vz + campos->vz;
		Test16Bit(&Test16bitX, &Test16bitY, &Test16bitZ);
		...
		vector[ 2 ] = *( vertices + face->vertex[ 2 ] );
		Test16bitX = vector[2].vx + campos->vx;
	    Test16bitY = vector[2].vy + campos->vy;
	    Test16bitZ = vector[2].vz + campos->vz;
		Test16Bit(&Test16bitX, &Test16bitY, &Test16bitZ);
		...
		vector[ 3 ] = *( vertices + face->vertex[ 3 ] );
		Test16bitX = vector[3].vx + campos->vx;
	    Test16bitY = vector[3].vy + campos->vy;
	    Test16bitZ = vector[3].vz + campos->vz;
		Test16Bit(&Test16bitX, &Test16bitY, &Test16bitZ);
...
//         if ( !( ( flag[ 0 ] | flag[ 1 ] | flag[ 2 ] | flag[ 3 ] ) & ShortClipFlags ))
         if ( !( ( flag[ 0 ] | flag[ 1 ] | flag[ 2 ] | flag[ 3 ] ) & (ZNegative >> 12) ))
         {
			if (!((flag [0] & ZReject) && (flag[1] & ZReject) && (flag[2] & ZReject) && (flag[3] & ZReject)))
			{
	            f4 = (PolyF4 *)&( TrackHi[ CurrentScreen ][ TrackHiCount++ ] );
...
//				if (NormalClip(*(long *)&screen[0],*(long *)&screen[1],*(long *)&screen[2])< 0)
//				if (NormalClip(screen[0],screen[1],screen[2]) >= 0)
//					continue;

				furthestDepth = depth[0];

				if (depth[1] < furthestDepth)
					furthestDepth = depth[1];
				if (depth[2] < furthestDepth)
					furthestDepth = depth[2];
				if (depth[3] < furthestDepth)
					furthestDepth = depth[3];

				furthestDepth += 2000;

				if ((furthestDepth >= 0) && (furthestDepth <= 8192))
		            //AddPrim( OT[ CurrentScreen ] + ((furthestDepth >> DepthShift) - DepthMod  ), ( ulong* ) f4 );
		            ClipPolyF4( OT[ CurrentScreen ] + ((furthestDepth >> DepthShift) - DepthMod  ), ( ulong* ) f4 );
			}
         }
...
TrackSection *AsmTransformTrackHiNoTex2
( 
   TrackSection*     firstSection,
   Skeleton*         camera,
   AutoMeshData*     autoMesh,
...
/* Calculate Med res meshed track sections */
   ...
   for ( i=0; i < highCount; i++ )
   {
      ...
      /* Top left polygon */
         ...
         if ( !( ( flag[ 0 ] | flag[ 1 ] | flag[ 3 ] | flag[ 4 ] ) & (ShortClipFlags & !ZNegative)))
         {
            f4 = (PolyF4 *)&( TrackHi[ CurrentScreen ][ TrackHiCount++ ] );
            SetPolyF4( ( POLY_F4* ) f4 );
            ...
            tile = &( surface->high[ 0 ] );	  // PCwipeout, was 2
			...
			if (depth[3] < furthestDepth)
				furthestDepth = depth[3];
			if (depth[4] < furthestDepth)
				furthestDepth = depth[4];

			realdepth = (furthestDepth >> DepthShift) + 200;
			if(realdepth > 8192)
				realdepth = 8192;

            //AddPrim( OT[ CurrentScreen ] + realdepth, ( ulong* ) f4 );
            ClipPolyF4( OT[ CurrentScreen ] + realdepth, ( ulong* ) f4 );
       }

      /* Top right polygon */

         if ( !( ( flag[ 1 ] | flag[ 2 ] | flag[ 4 ] | flag[ 5 ] ) & (ShortClipFlags & !ZNegative) ) )
         {
            f4 = (PolyF4 *)&( TrackHi[ CurrentScreen ][ TrackHiCount++ ] );
            ...
            tile = &( surface->high[ 1 ] ); // PCwipeout, was 3
			...
			if (depth[4] < furthestDepth)
				furthestDepth = depth[4];
			if (depth[5] < furthestDepth)
				furthestDepth = depth[5];
      ...
      /* Bottom left polygon */
         ...
         if ( !( ( flag[ 3 ] | flag[ 4 ] | flag[ 6 ] | flag[ 7 ] ) & (ShortClipFlags & !ZNegative)) )
         {
            f4 = (PolyF4 *)&( TrackHi[ CurrentScreen ][ TrackHiCount++ ] );
            ...
            tile = &( surface->high[ 2 ] ); // PCwipeout, was 0
			...
			if (depth[4] < furthestDepth)
				furthestDepth = depth[4];
			if (depth[6] < furthestDepth)
				furthestDepth = depth[6];
			if (depth[7] < furthestDepth)
				furthestDepth = depth[7];
      ...
      /* Bottom right polygon */
         ...
         if ( !( ( flag[ 4 ] | flag[ 5 ] | flag[ 7 ] | flag[ 8 ] ) & (ShortClipFlags & !ZNegative)) )
         {
            f4 = (PolyF4 *)&( TrackHi[ CurrentScreen ][ TrackHiCount++ ] );
            ...
            tile = &( surface->high[ 3 ] ); // PCwipeout, was 1
			...
			if (depth[7] < furthestDepth)
				furthestDepth = depth[7];
			if (depth[8] < furthestDepth)
				furthestDepth = depth[8];
...
TrackSection *AsmTransformTrackHiNoTex
( 
   TrackSection*     firstSection,
   Skeleton*         camera,
   AutoMeshData*     autoMesh,
   ...
   for ( i=0; i < highCount; i++ )
   {
      ...
      /* polygon 0 */
         ...
         else if ( !( flag[ 0 ] & flag[ 1 ] & flag[ 5 ] & flag[ 6 ] & ShortClipFlags ) )
         {
            ...
            AsmAutoMeshNoTex( &svector[ 0 ], &svector[ 1 ], &svector[ 5 ], &svector[ 6 ], face, tile, autoMesh );
         }
      ...
      /* polygon 1 */
         ...
         if ( !( ( flag[ 1 ] | flag[ 2 ] | flag[ 6 ] | flag[ 7 ] ) & ShortClipFlags ) )
         {
            ...
            //AddPrim( OT[ CurrentScreen ] + (( depth[ 1 ] >> DepthShift) - DepthMod  ), ( ulong* ) f4 );
            ClipPolyF4( OT[ CurrentScreen ] + (( depth[ 1 ] >> DepthShift) - DepthMod  ), ( ulong* ) f4 );
         }
         else if ( !( flag[ 1 ] & flag[ 2 ] & flag[ 6 ] & flag[ 7 ] & ShortClipFlags ) )
         {
            if ( face->flags & FlipTexture )
            ...
            AsmAutoMeshNoTex( &svector[ 1 ], &svector[ 2 ], &svector[ 6 ], &svector[ 7 ], face, tile, autoMesh );
         }
      ...
      /* polygon 2 */
         ...
         if ( !( ( flag[ 2 ] | flag[ 3 ] | flag[ 7 ] | flag[ 8 ] ) & ShortClipFlags ) )
         {
            ...
            //AddPrim( OT[ CurrentScreen ] + (( depth[ 2 ] >> DepthShift) - DepthMod  ), ( ulong* ) f4 );
            ClipPolyF4( OT[ CurrentScreen ] + (( depth[ 2 ] >> DepthShift) - DepthMod  ), ( ulong* ) f4 );
         }
         else if ( !( flag[ 2 ] & flag[ 3 ] & flag[ 7 ] & flag[ 8 ] & ShortClipFlags ) )
         {
            if ( face->flags & FlipTexture )
            ...
            AsmAutoMeshNoTex( &svector[ 2 ], &svector[ 3 ], &svector[ 7 ], &svector[ 8 ], face, tile, autoMesh );
         }
      ...
      /* polygon 3 */
         ...
         if ( !( ( flag[ 3 ] | flag[ 4 ] | flag[ 8 ] | flag[ 9 ] ) & ShortClipFlags ) )
         {
            ...
            //AddPrim( OT[ CurrentScreen ] + (( depth[ 3 ] >> DepthShift) - DepthMod  ), ( ulong* ) f4 );
            ClipPolyF4( OT[ CurrentScreen ] + (( depth[ 3 ] >> DepthShift) - DepthMod  ), ( ulong* ) f4 );
         }
         else if ( !( flag[ 3 ] & flag[ 4 ] & flag[ 8 ] & flag[ 9 ] & ShortClipFlags ) )
         {
            if ( face->flags & FlipTexture )
            ...
            AsmAutoMeshNoTex( &svector[ 3 ], &svector[ 4 ], &svector[ 8 ], &svector[ 9 ], face, tile, autoMesh );
         }
      ...
      /* polygon 4 */
         ...
         if ( !( ( flag[ 5 ] | flag[ 6 ] | flag[ 10 ] | flag[ 11 ] ) & ShortClipFlags ) )
         {
            ...
            //AddPrim( OT[ CurrentScreen ] + (( depth[ 5 ] >> DepthShift) - DepthMod  ), ( ulong* ) f4 );
            ClipPolyF4( OT[ CurrentScreen ] + (( depth[ 5 ] >> DepthShift) - DepthMod  ), ( ulong* ) f4 );
         }
         else if ( !( flag[ 5 ] & flag[ 6 ] & flag[ 10 ] & flag[ 11 ] & ShortClipFlags ) )
         {
            if ( face->flags & FlipTexture )
            ...
            AsmAutoMeshNoTex( &svector[ 5 ], &svector[ 6 ], &svector[ 10 ], &svector[ 11 ], face, tile, autoMesh );
         }
      ...
      /* polygon 5 */
         ...
         //   AddPrim( OT[ CurrentScreen ] + (( depth[ 6 ] >> DepthShift) - DepthMod  ), ( ulong* ) f4 );
            ClipPolyF4( OT[ CurrentScreen ] + (( depth[ 6 ] >> DepthShift) - DepthMod  ), ( ulong* ) f4 );
         }
         else if ( !( flag[ 6 ] & flag[ 7 ] & flag[ 11 ] & flag[ 12 ] & ShortClipFlags ) )
         {
            if ( face->flags & FlipTexture )
            ...
            AsmAutoMeshNoTex( &svector[ 6 ], &svector[ 7 ], &svector[ 11 ], &svector[ 12 ], face, tile, autoMesh );
         }
      ...
      /* polygon 6 */
         ...
         if ( !( ( flag[ 7 ] | flag[ 8 ] | flag[ 12 ] | flag[ 13 ] ) & ShortClipFlags ) )
         {
            ...
            //AddPrim( OT[ CurrentScreen ] + (( depth[ 7 ] >> DepthShift) - DepthMod  ), ( ulong* ) f4 );
            ClipPolyF4( OT[ CurrentScreen ] + (( depth[ 7 ] >> DepthShift) - DepthMod  ), ( ulong* ) f4 );
         }
         else if ( !( flag[ 7 ] & flag[ 8 ] & flag[ 12 ] & flag[ 13 ] & ShortClipFlags ) )
         {
            if ( face->flags & FlipTexture )
            ...
            AsmAutoMeshNoTex( &svector[ 7 ], &svector[ 8 ], &svector[ 12 ], &svector[ 13 ], face, tile, autoMesh );
         }
      ...
      /* polygon 7 */
         ...
         if ( !( ( flag[ 8 ] | flag[ 9 ] | flag[ 13 ] | flag[ 14 ] ) & ShortClipFlags ) )
         {
            ...
            //AddPrim( OT[ CurrentScreen ] + (( depth[ 8 ] >> DepthShift) - DepthMod  ), ( ulong* ) f4 );
            ClipPolyF4( OT[ CurrentScreen ] + (( depth[ 8 ] >> DepthShift) - DepthMod  ), ( ulong* ) f4 );
         }
         else if ( !( flag[ 8 ] & flag[ 9 ] & flag[ 13 ] & flag[ 14 ] & ShortClipFlags ) )
         {
            if ( face->flags & FlipTexture )
            ...
            AsmAutoMeshNoTex( &svector[ 8 ], &svector[ 9 ], &svector[ 13 ], &svector[ 14 ], face, tile, autoMesh );
         }
      ...
      /* polygon 8 */
         ...
         if ( !( ( flag[ 10 ] | flag[ 11 ] | flag[ 15 ] | flag[ 16 ] ) & ShortClipFlags ) )
         {
            ...
            //AddPrim( OT[ CurrentScreen ] + (( depth[ 10 ] >> DepthShift) - DepthMod  ), ( ulong* ) f4 );
            ClipPolyF4( OT[ CurrentScreen ] + (( depth[ 10 ] >> DepthShift) - DepthMod  ), ( ulong* ) f4 );
         }
         else if ( !( flag[ 10 ] & flag[ 11 ] & flag[ 15 ] & flag[ 16 ] & ShortClipFlags ) )
         {
            if ( face->flags & FlipTexture )
            ...
            AsmAutoMeshNoTex( &svector[ 10 ], &svector[ 11 ], &svector[ 15 ], &svector[ 16 ], face, tile, autoMesh );
         }
      ...
      /* polygon 9 */
         ...
         if ( !( ( flag[ 11 ] | flag[ 12 ] | flag[ 16 ] | flag[ 17 ] ) & ShortClipFlags ) )
         {
            ...
            //AddPrim( OT[ CurrentScreen ] + (( depth[ 11 ] >> DepthShift) - DepthMod  ), ( ulong* ) f4 );
            ClipPolyF4( OT[ CurrentScreen ] + (( depth[ 11 ] >> DepthShift) - DepthMod  ), ( ulong* ) f4 );
         }
         else if ( !( flag[ 11 ] & flag[ 12 ] & flag[ 16 ] & flag[ 17 ] & ShortClipFlags ) )
         {
            if ( face->flags & FlipTexture )
            ...
            AsmAutoMeshNoTex( &svector[ 11 ], &svector[ 12 ], &svector[ 16 ], &svector[ 17 ], face, tile, autoMesh );
         }
      ...
      /* polygon 10 */
         ...
         if ( !( ( flag[ 12 ] | flag[ 13 ] | flag[ 17 ] | flag[ 18 ] ) & ShortClipFlags ) )
         {
            ...
            //AddPrim( OT[ CurrentScreen ] + (( depth[ 12 ] >> DepthShift) - DepthMod  ), ( ulong* ) f4 );
            ClipPolyF4( OT[ CurrentScreen ] + (( depth[ 12 ] >> DepthShift) - DepthMod  ), ( ulong* ) f4 );
         }
         else if ( !( flag[ 12 ] & flag[ 13 ] & flag[ 17 ] & flag[ 18 ] & ShortClipFlags ) )
         {
            if ( face->flags & FlipTexture )
            ...
            AsmAutoMeshNoTex( &svector[ 12 ], &svector[ 13 ], &svector[ 17 ], &svector[ 18 ], face, tile, autoMesh );
         }
      ...
      /* polygon 11 */
         ...
         if ( !( ( flag[ 13 ] | flag[ 14 ] | flag[ 18 ] | flag[ 19 ] ) & ShortClipFlags ) )
         {
            ...
            //AddPrim( OT[ CurrentScreen ] + (( depth[ 13 ] >> DepthShift) - DepthMod  ), ( ulong* ) f4 );
            ClipPolyF4( OT[ CurrentScreen ] + (( depth[ 13 ] >> DepthShift) - DepthMod  ), ( ulong* ) f4 );
         }
         else if ( !( flag[ 13 ] & flag[ 14 ] & flag[ 18 ] & flag[ 19 ] & ShortClipFlags ) )
         {
            if ( face->flags & FlipTexture )
            ...
            AsmAutoMeshNoTex( &svector[ 13 ], &svector[ 14 ], &svector[ 18 ], &svector[ 19 ], face, tile, autoMesh );
         }
      ...
      /* polygon 12 */
         ...
         if ( !( ( flag[ 15 ] | flag[ 16 ] | flag[ 20 ] | flag[ 21 ] ) & ShortClipFlags ) )
         {
            ...
            //AddPrim( OT[ CurrentScreen ] + (( depth[ 15 ] >> DepthShift) - DepthMod  ), ( ulong* ) f4 );
            ClipPolyF4( OT[ CurrentScreen ] + (( depth[ 15 ] >> DepthShift) - DepthMod  ), ( ulong* ) f4 );
         }
         else if ( !( flag[ 15 ] & flag[ 16 ] & flag[ 20 ] & flag[ 21 ] & ShortClipFlags ) )
         {
            if ( face->flags & FlipTexture )
            ...
            AsmAutoMeshNoTex( &svector[ 15 ], &svector[ 16 ], &svector[ 20 ], &svector[ 21 ], face, tile, autoMesh );
         }
      ...
      /* polygon 13 */
         ...
         if ( !( ( flag[ 16 ] | flag[ 17 ] | flag[ 21 ] | flag[ 22 ] ) & ShortClipFlags ) )
         {
            ...
            //AddPrim( OT[ CurrentScreen ] + (( depth[ 16 ] >> DepthShift) - DepthMod  ), ( ulong* ) f4 );
            ClipPolyF4( OT[ CurrentScreen ] + (( depth[ 16 ] >> DepthShift) - DepthMod  ), ( ulong* ) f4 );
         }
         else if ( !( flag[ 16 ] & flag[ 17 ] & flag[ 21 ] & flag[ 22 ] & ShortClipFlags ) )
         {
            if ( face->flags & FlipTexture )
            ...
            AsmAutoMeshNoTex( &svector[ 16 ], &svector[ 17 ], &svector[ 21 ], &svector[ 22 ], face, tile, autoMesh );
         }
      ...
      /* polygon 14 */
         ...
         if ( !( ( flag[ 17 ] | flag[ 18 ] | flag[ 22 ] | flag[ 23 ] ) & ShortClipFlags ) )
         {
            ...
            //AddPrim( OT[ CurrentScreen ] + (( depth[ 17 ] >> DepthShift) - DepthMod  ), ( ulong* ) f4 );
            ClipPolyF4( OT[ CurrentScreen ] + (( depth[ 17 ] >> DepthShift) - DepthMod  ), ( ulong* ) f4 );
         }
         else if ( !( flag[ 17 ] & flag[ 18 ] & flag[ 22 ] & flag[ 23 ] & ShortClipFlags ) )
         {
            if ( face->flags & FlipTexture )
            ...
            AsmAutoMeshNoTex( &svector[ 17 ], &svector[ 18 ], &svector[ 22 ], &svector[ 23 ], face, tile, autoMesh );
         }
      ...
      /* polygon 15 */
         ...
         if ( !( ( flag[ 18 ] | flag[ 19 ] | flag[ 23 ] | flag[ 24 ] ) & ShortClipFlags ) )
         {
            ...
            //AddPrim( OT[ CurrentScreen ] + (( depth[ 18 ] >> DepthShift) - DepthMod  ), ( ulong* ) f4 );
            ClipPolyF4( OT[ CurrentScreen ] + (( depth[ 18 ] >> DepthShift) - DepthMod  ), ( ulong* ) f4 );
         }
         else if ( !( flag[ 18 ] & flag[ 19 ] & flag[ 23 ] & flag[ 24 ] & ShortClipFlags ) )
         {
            if ( face->flags & FlipTexture )
            ...
            AsmAutoMeshNoTex( &svector[ 18 ], &svector[ 19 ], &svector[ 23 ], &svector[ 24 ], face, tile, autoMesh );
         }
...
)
{
...
#if 0
   SVECTOR*       svector =     ( SVECTOR* )( FastRam + 524 );
   DVECTOR*       screen =      ( DVECTOR* )( FastRam + 596 ); 
   long*          depth =       ( long* )   ( FastRam + 632 );    
   long*          flag =        ( long* )   ( FastRam + 668 );    
   long*          interpolate = ( long* )   ( FastRam + 704 );    
   uchar*         uv =          ( uchar* )  ( FastRam + 740 );
#else
   ...
   if ( ( autoMesh->quadCount >= MaxAutos ) || ( autoMesh->triCount >= MaxAutos ) )
   {
      ...
      FntPrint( "too many automeshed polys\n" );
...
/* Check if centre coord is valid */
   ...
   /* Meshing the quad into quarters */
      ...
      if ( clip & topLeft )
      {
         if ( ( clip & top ) && ( clip & left ) )
         {
            ...
            ClipPolyFT4( OT[ CurrentScreen ] + depth[ cNum ], ( ulong* )( ft4 ) );
         ...
         else if ( clip & top )
         {
            ...
            ClipPolyFT3( OT[ CurrentScreen ] + depth[ cNum ], ( ulong* )( ft3 ) );
   ...
   {
      if ( 0 && ( clip & topLeft ) && ( clip & top ) && ( clip & left ) )
      {
         ft3 = &( autoMesh->tri[ CurrentScreen ][ autoMesh->triCount ] );
         ...
         //ft3->xy1 = screen[ tNum ];
         //ft3->xy2 = screen[ cNum ];
            ft3->xy0.vx = screen[ tlNum ].vx;
            ft3->xy0.vy = screen[ tlNum ].vy;
            ft3->xy1.vx = screen[ tNum ].vx;
            ft3->xy1.vy = screen[ tNum ].vy;
            ft3->xy2.vx = screen[ cNum ].vx;
            ft3->xy2.vy = screen[ cNum ].vy;
			ft3->xy0.vz = depth[tlNum];
			ft3->xy1.vz = depth[tNum];
			ft3->xy2.vz = depth[cNum];
			ft3->xy0.pad = 1;
			ft3->xy1.pad = 1;
			ft3->xy2.pad = 1;
      ...
      if ( ( clip & topRight ) && ( clip & top ) && ( clip & right ) )
      {
         ft3 = &( autoMesh->tri[ CurrentScreen ][ autoMesh->triCount ] );
         ...
         //ft3->xy2 = screen[ tNum ];
            ft3->xy0.vx = screen[ trNum ].vx;
            ft3->xy0.vy = screen[ trNum ].vy;
            ft3->xy1.vx = screen[ rNum ].vx;
            ft3->xy1.vy = screen[ rNum ].vy;
            ft3->xy2.vx = screen[ tNum ].vx;
            ft3->xy2.vy = screen[ tNum ].vy;
			ft3->xy0.vz = depth[trNum];
			ft3->xy1.vz = depth[rNum];
			ft3->xy2.vz = depth[tNum];
			ft3->xy0.pad = 1;
			ft3->xy1.pad = 1;
			ft3->xy2.pad = 1;
      ...
      if ( ( clip & bottomLeft ) && ( clip & bottom ) && ( clip & left ) )
      {
      }

      if ( ( clip & bottomRight ) && ( clip & bottom ) && ( clip & right ) )
      {
      }
   }
#endif
...
/* Check if centre coord is valid */
   ...
   /* Meshing the quad into quarters */
      ...
      if ( clip & topLeft )
      {
         if ( ( clip & top ) && ( clip & left ) )
         {
            f4 = (PolyF4 *)&( autoMesh->quad[ CurrentScreen ][ autoMesh->quadCount ] );
            ...
            //AddPrim( OT[ CurrentScreen ] + depth[ cNum ], ( ulong* )( f4 ) );
            ClipPolyF4( OT[ CurrentScreen ] + depth[ cNum ], ( ulong* )( f4 ) );

            autoMesh->quadCount += 1;
         }
         else if ( clip & top )
         {
            f3 = (PolyF3 *)&( autoMesh->tri[ CurrentScreen ][ autoMesh->triCount ] );

            f3->xy0 = screen[ tlNum ];
            f3->xy1 = screen[ tNum ];
            f3->xy2 = screen[ cNum ];

			f3->colour0.r = face->red;
            f3->colour0.g = 0;
            f3->colour0.b = 0;

            SetPolyF3( ( POLY_F3* )( f3 ) );

            //AddPrim( OT[ CurrentScreen ] + depth[ cNum ], ( ulong* )( f3 ) );
            ClipPolyF3( OT[ CurrentScreen ] + depth[ cNum ], ( ulong* )( f3 ) );
