 ...
 */
static char DefaultGameDirectory[MAX_PATH] = "C:\\3D Rage Games\\Assault Rigs";

/*
 * title of game installation
 */
static char title [] = "Assault Rigs for 3D Rage Setup";
...
int MakeShortcut(char *szTargetName, char *szLinkName, char *szTargetDir)
{
    ...
    strcpy(szLinkGroupPathName, szWindowsDir);
	...
	/* Build path name for link in start up menu */
    strcpy(szLinkPathName, szWindowsDir);
...
int GetDiskRequirements(char *szRootPath)
{
	...
	retval = GetFileSpaceRequired((char **)pszStandardFiles, gdDiskClusterSize, &gdStandardFileSpace);
	if (retval != 0)
	{
		return(-1);
	}
	retval = GetFileSpaceRequired((char **)pszSaveGameFiles, gdDiskClusterSize, &dwTemp);
	if (retval != 0)
	{
		return(-1);
	}
	...
	retval = GetFileSpaceRequired((char **)&pszDataFiles, gdDiskClusterSize, &gdDataFileSpace);
	if (retval != 0)
	{
		return(-1);
	}
	...
	retval = GetFileSpaceRequired((char **)&pszMovieFiles, gdDiskClusterSize, &gdMovieFileSpace);
	if (retval != 0)
	{
		return(-1);
	}
