...
void UpdateCameraCollideTest(TrackCamera *camera, ShipData *camShip, RescueData *rescueDroid)
{
   VECTOR 	pos;
   short    sx, sy, sz;
   short    cx, cy, cz;
   ...
   cz = rcos( camShip->roll );
	...
	pos.vx = camShip->ppivot.vx;
	pos.vy = camShip->ppivot.vy - 1500;
	pos.vz = camShip->ppivot.vz;
...
void SetCameraToRedPoly(TrackCamera *camera, Face *facePtr)
{
   ...
   av.vx = ( track->vertices[(facePtr->vertex[0])].vx + track->vertices[(facePtr->vertex[1])].vx +
            track->vertices[(facePtr->vertex[2])].vx + track->vertices[(facePtr->vertex[3])].vx ) / 4;

   av.vy = ( track->vertices[(facePtr->vertex[0])].vy + track->vertices[(facePtr->vertex[1])].vy +
            track->vertices[(facePtr->vertex[2])].vy + track->vertices[(facePtr->vertex[3])].vy ) / 4;

   av.vz = ( track->vertices[(facePtr->vertex[0])].vz + track->vertices[(facePtr->vertex[1])].vz +
            track->vertices[(facePtr->vertex[2])].vz + track->vertices[(facePtr->vertex[3])].vz ) / 4;
